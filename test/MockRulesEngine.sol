...
contract MockRulesEngine is IRulesEngine {
    function checkPolicies(bytes calldata arguments) external pure override {
        ...
        ) = abi.decode(arguments[4:], (address, address, address, uint256, uint256, uint256, uint256, uint256, uint256, uint256));
        ...
        if (treasuryBypass != 1 && blockTime < tokenUnlockTime) {
            revert("Token still locked");
        }
